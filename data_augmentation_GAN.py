...
# CONFIGURACIÓN
# ==========================================
# Usamos tamano de imagen 64x64 que es estable y no necesitamos mas
# Vector de ruido de tamano 128
# Epoch 50, batch size 32 por ahora.
...
# imagenet_dataset_from_directory carga las imagenes y etiqueta de la carpeta.
...
def build_generator(latent_dim, num_classes):
    ...
    label_embedding = layers.Dense(4 * 4 * 1)(label_embedding)
    ...
    # Vector de ruido asociado a cada etiqueta
    merge = layers.Concatenate()([noise_gen, label_embedding])

    # Upsampling (Aumentar resolución)
...
def train_step(real_images, labels):
    ...
    # Etiquetas para el discriminador (1=real, 0=falsa)
    ...
    with tf.GradientTape() as tape:
        ...
        # El objetivo será "engañar" al discriminador
        predictions_fake = discriminator([generated_images, labels], training=True)
...
IMAGES_PER_CLASS = 50 # por ahora 50 imagenes generadas pro clase
...
print(f"FIN. Imágenes guardadas en la carpeta '{OUTPUT_FOLDER}'.")
